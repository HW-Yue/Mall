...
class DynamicTpRefreshTest {
    ...
    void shouldRefreshBusinessExecutorAndTomcatExecutor() {
        ...
        DtpExecutor executor = new DtpExecutor(2, 50, 5, TimeUnit.SECONDS, new LinkedBlockingQueue<>());
        executor.setThreadPoolName("threadPoolExecutor");
        executor.setThreadPoolAliasName("mall-business");
        ...
        executorProps.setCorePoolSize(6);
        executorProps.setMaximumPoolSize(60);
        executorProps.setKeepAliveTime(30);
        ...
        assertEquals(60, executor.getMaximumPoolSize());
    ...
    private static void printDtpState(String phase, DtpExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-mall] phase=%s dtpCore=%d dtpMax=%d dtpKeepAlive=%d%n",
    ...
    private static void printTomcatState(String phase, org.apache.tomcat.util.threads.ThreadPoolExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-mall] phase=%s tomcatCore=%d tomcatMax=%d tomcatKeepAlive=%d%n",
                phase, executor.getCorePoolSize(), executor.getMaximumPoolSize(),
                executor.getKeepAliveTime(TimeUnit.SECONDS));
    }
