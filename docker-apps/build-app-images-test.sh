...
if [[ "${SKIP_MAVEN:-false}" != "true" ]]; then
  echo "[1/2] Packaging application jars for test profile Docker flow..."
  mvn -pl "$(IFS=,; echo "${MODULES[*]}")" -am clean package -DskipTests
else
  echo "[1/2] SKIP_MAVEN=true, using existing target jars."
fi

echo "[2/2] Building Docker images from dev-ops/docker-compose-apps-test.yml..."
echo "Including frontend images defined in compose, such as mall-frontend and ops-agent-frontend."
