...
curl -sS -X POST "${BASE_URL}/api/v1/alert/receive" \
  ...
  -d "$(ALERT_KEY='RedisDown' python3 -c "import json,os; p=os.environ['BODIES_JSON']; k=os.environ['ALERT_KEY']; d=json.load(open(p,encoding='utf-8')); print(json.dumps(d[k], ensure_ascii=False))")"
echo ""
