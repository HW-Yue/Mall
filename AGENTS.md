...
### 拼团下单
前端 → group-buy-service `create_pay_order`（校验活动、占 `lock_count`、必要时创建 team）→ Feign order-service `create_order` → order-service 写 Redis 存在标记并发布 `group-buy-order-create` → 立即返回 `orderId` / `teamId` / `outTradeNo` → order-service 消费后异步落库 `t_order` → order-service `get_pay_url`（命中存在标记后做 DB 重试兜底）→ Feign pay → 支付宝
