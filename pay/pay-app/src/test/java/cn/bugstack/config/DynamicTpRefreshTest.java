...
class DynamicTpRefreshTest {
    ...
    void shouldRefreshBusinessExecutorAndTomcatExecutor() {
        ...
        DtpExecutor executor = new DtpExecutor(20, 50, 5000, TimeUnit.SECONDS, new LinkedBlockingQueue<>());
        executor.setThreadPoolName("threadPoolExecutor");
        executor.setThreadPoolAliasName("pay-business");
        ...
        executorProps.setCorePoolSize(24);
        executorProps.setMaximumPoolSize(64);
        executorProps.setKeepAliveTime(3600);
        ...
        tomcatTp.setCorePoolSize(30);
        tomcatTp.setMaximumPoolSize(260);
        ...
        assertEquals(30, tomcatExecutor.getCorePoolSize());
        assertEquals(260, tomcatExecutor.getMaximumPoolSize());
    ...
    private static void printDtpState(String phase, DtpExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-pay] phase=%s dtpCore=%d dtpMax=%d dtpKeepAlive=%d%n",
    ...
    private static void printTomcatState(String phase, org.apache.tomcat.util.threads.ThreadPoolExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-pay] phase=%s tomcatCore=%d tomcatMax=%d tomcatKeepAlive=%d%n",
                phase, executor.getCorePoolSize(), executor.getMaximumPoolSize(),
                executor.getKeepAliveTime(TimeUnit.SECONDS));
    }
