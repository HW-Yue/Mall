...
/**
 * 消费普通订单关单消息，关闭 pay_order。
 */
@Slf4j
@Component
@ConditionalOnProperty(prefix = "rocketmq", name = "name-server")
@RocketMQMessageListener(
        topic = "${rocketmq.topic.order_close_normal:order-close-normal}",
        consumerGroup = "${rocketmq.consumerGroup.orderCloseNormal:CG_PAY_ORDER_CLOSE_NORMAL}"
)
public class OrderCloseNormalListener implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
        log.info("pay-service 收到普通订单关单消息: {}", message);
        try {
            ...
            if (OrderStatusVO.PAY_WAIT.getCode().equals(status)) {
                ...
                log.warn("pay-service 发现普通订单已支付但收到关单消息，执行退款 outTradeNo:{}", outTradeNo);
                orderService.refundPayOrderByOutTradeNo(outTradeNo);
            } else {
                log.info("pay-service 普通支付单状态无需处理 outTradeNo:{} status:{}", outTradeNo, status);
            }
        } catch (AlipayApiException e) {
            log.error("pay-service 普通订单关单支付宝调用失败: {}", message, e);
            throw new RuntimeException(e);
        } catch (Exception e) {
            log.error("pay-service 普通订单关单处理失败: {}", message, e);
            throw new RuntimeException(e);
        }
    }
}
