...
public class LoginPort implements ILoginPort {
    ...
    private static final String PASSWORD_SALT = "s-pay-mall-user";
    ...
    public String queryUsernameByOpenid(String openid) {
        ...
        UserAccount user = userDao.queryByOpenid(openid);
        return user != null ? user.getUsername() : null;
    }

    @Override
    public void registerUser(String openid, String username, String password) {
        if (userDao.queryByUsername(username) != null) {
            throw new IllegalArgumentException("用户名已存在");
        }
        String pwdHash = DigestUtil.sha256Hex(PASSWORD_SALT + password);
