...
class RocketMqConsumerThreadPoolRefresherTest {
    ...
    void shouldUpdateRunningConsumerThreadPoolOnStartupAndRefresh() {
        String consumerGroup = "CG_NORMAL_ORDER_CREATE";
        ...
        beanFactory.addBean("normalOrderCreateListenerContainer", container);

        RocketMqConsumerThreadPoolProperties properties = new RocketMqConsumerThreadPoolProperties();
        properties.setConsumers(Map.of(
                consumerGroup, consumerConfig(20, 64)
        ));
        ...
        properties.setConsumers(Map.of(
                consumerGroup, consumerConfig(8, 16)
        ));
        ...
        refresher.onApplicationEvent(new EnvironmentChangeEvent(this, Set.of(
                "app.rocketmq.consumer-thread-pool.consumers.CG_NORMAL_ORDER_CREATE.consume-thread-number",
                "app.rocketmq.consumer-thread-pool.consumers.CG_NORMAL_ORDER_CREATE.consume-thread-max"
        )));
    ...
    private static void printState(String phase, DefaultMQPushConsumer consumer, ThreadPoolExecutor executor) {
        System.out.printf(
                "[RocketMqConsumerThreadPoolRefresherTest] phase=%s consumerMin=%d consumerMax=%d executorCore=%d executorMax=%d%n",
                phase,
