...
class DynamicTpRefreshTest {
    ...
    void shouldRefreshBusinessExecutorAndTomcatExecutor() {
        ...
        DtpExecutor executor = new DtpExecutor(5, 20, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<>());
        executor.setThreadPoolName("threadPoolExecutor");
        executor.setThreadPoolAliasName("order-business");
        ...
        executorProps.setCorePoolSize(8);
        executorProps.setMaximumPoolSize(28);
        ...
        assertEquals(28, executor.getMaximumPoolSize());
    ...
    private static void printDtpState(String phase, DtpExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-order] phase=%s dtpCore=%d dtpMax=%d dtpKeepAlive=%d%n",
    ...
    private static void printTomcatState(String phase, org.apache.tomcat.util.threads.ThreadPoolExecutor executor) {
        System.out.printf("[DynamicTpRefreshTest-order] phase=%s tomcatCore=%d tomcatMax=%d tomcatKeepAlive=%d%n",
                phase, executor.getCorePoolSize(), executor.getMaximumPoolSize(),
                executor.getKeepAliveTime(TimeUnit.SECONDS));
    }
