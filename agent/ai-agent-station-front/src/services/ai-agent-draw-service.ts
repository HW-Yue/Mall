...
export class AiAgentDrawService {
  ...
  /**
   * 根据策略获取该策略允许的 client_type 槽位列表
   * @param strategy 策略值，如 flowAgentExecuteStrategy
   * @returns client_type 编码数组，如 ['TASK_ANALYZER_CLIENT', 'PRECISION_EXECUTOR_CLIENT']
   */
  static async getClientTypesByStrategy(strategy: string): Promise<string[]> {
    try {
      const url = `${this.BASE_URL}${API_ENDPOINTS.AI_AGENT_DRAW.CLIENT_TYPES_BY_STRATEGY}?strategy=${encodeURIComponent(strategy)}`;
      const response = await fetch(url, { method: 'GET', headers: DEFAULT_HEADERS });
      if (!response.ok) return getClientTypesByStrategyFallback(strategy);
      const result: ApiResponse<string[]> = await response.json();
      if (result.code === '0000' && Array.isArray(result.data)) return result.data;
      return getClientTypesByStrategyFallback(strategy);
    } catch {
      return getClientTypesByStrategyFallback(strategy);
    }
  }
...
/** 前端兜底：策略 -> client_type 槽位映射（后端未提供接口时使用） */
function getClientTypesByStrategyFallback(strategy: string): string[] {
