  ...
  const columns = [
    {
      ...
      key: 'mcpNameId',
      width: 200,
      render: (_: unknown, record: AiClientToolMcpResponseDTO) => (
        <NameIdCell
          name={record.mcpName}
          id={record.mcpId}
          idLabel="MCP ID"
    ...
    },
    {
      ...
      render: (config: string, record: AiClientToolMcpResponseDTO) => (
        <ConfigPreview onClick={(e) => { e.stopPropagation(); showConfigDetail(config || '', record.mcpName); }}>
          {config ? (config.length > 50 ? `${config.substring(0, 50)}...` : config) : '-'}
        </ConfigPreview>
      ),
    },
    {
      ...
      render: (type: string) =>
        type ? (
          <Tag color={type === 'stdio' ? 'blue' : type === 'sse' ? 'green' : 'orange'}>{type}</Tag>
        ) : (
          '-'
        ),
    },
    ...
    },
    {
      ...
      render: (_: unknown, record: AiClientToolMcpResponseDTO) => (
        <TableActionLinks
          onView={() => showConfigDetail(record.transportConfig, record.mcpName)}
          viewLabel="查看配置"
          onEdit={() => handleEdit(record)}
          onDelete={() => handleDelete(record)}
          deleteConfirmTitle="确定要删除这个MCP配置吗？"
