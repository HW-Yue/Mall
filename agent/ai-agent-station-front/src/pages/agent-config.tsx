...
import { AgentConfigRunProvider } from '../contexts/agent-config-run-context';
import { AiAgentDrawService, AiAgentDrawConfigResponseDTO } from '../services/ai-agent-draw-service';
...
export const AgentConfigPage: React.FC = () => {
  ...
  const openDrawerWithConfig = (config: AiAgentDrawConfigResponseDTO) => {
    openDrawer({
  ...
  const handleTestRun = async () => {
    ...
    openDrawerWithConfig(config);
  };

  const openTestRunModal = async () => {
    if (!configId) {
      Toast.warning('请先保存配置后再测试运行');
      return;
    }
      ...
      <MainContent>
        <ContentWrapper $sidebarWidth={sidebarWidth}>
          ...
          <ContentArea>
            ...
            <EditorSection>
              <EditorContainer>
                <AgentConfigRunProvider value={{ onRunClick: openTestRunModal }}>
                  <Editor />
                </AgentConfigRunProvider>
              </EditorContainer>
            </EditorSection>
