...
export const RagOrderManagement: React.FC = () => {
  ...
  const columns = [
    {
      ...
      key: 'ragNameId',
      width: 200,
      render: (_: unknown, record: AiClientRagOrderResponseDTO) => (
        <NameIdCell
          name={record.ragName}
          id={record.ragId}
          idLabel="知识库ID"
    ...
    },
    {
      ...
      render: (_: unknown, record: AiClientRagOrderResponseDTO) => (
        <TableActionLinks
          ...
          deleteConfirmTitle="确定要删除这个知识库配置吗？"
  ...
  const handleUploadSubmit = async () => {
    try {
      ...
      // 从 Semi Upload 的 fileList 中提取 File（兼容 fileInstance / file）
      const files: File[] = fileList
        .map((item: { fileInstance?: File; file?: File }) => item.fileInstance ?? item.file)
      ...
      if (result.code === '0000' && result.data) {
        ...
        Toast.error(result.info || '上传失败');
      }
    } catch (error) {
      console.error('上传知识库文件失败:', error);
      Toast.error(error instanceof Error ? error.message : '上传失败，请检查网络连接');
    } finally {
      setUploadLoading(false);
    }
  };
