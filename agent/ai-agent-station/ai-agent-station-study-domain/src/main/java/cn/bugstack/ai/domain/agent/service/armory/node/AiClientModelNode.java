...
public class AiClientModelNode extends AbstractArmorySupport {
    ...
    protected String doApply(ArmoryCommandEntity requestParameter, DefaultArmoryStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        // 策略绑定：MCP 由客户端直连，从 context 取当前客户端的 MCP 列表（与 AiClientToolMcpNode 注册的 Bean 一致）
        List<AiClientToolMcpVO> clientMcpList = dynamicContext.getValue(AiAgentEnumVO.AI_CLIENT_TOOL_MCP.getDataName());
        if (clientMcpList == null) {
            clientMcpList = new ArrayList<>();
        }
        ...
        for (AiClientModelVO modelVO : aiClientModelList) {
            ...
            for (AiClientToolMcpVO mcpVO : clientMcpList) {
                McpSyncClient mcpSyncClient = getBean(AiAgentEnumVO.AI_CLIENT_TOOL_MCP.getBeanName(mcpVO.getMcpId()));
