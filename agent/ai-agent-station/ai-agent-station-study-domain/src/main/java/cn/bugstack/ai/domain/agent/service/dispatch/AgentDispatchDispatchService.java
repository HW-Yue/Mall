    ...
    public void dispatch(ExecuteCommandEntity requestParameter, ResponseBodyEmitter emitter) throws Exception {
        ...
        threadPoolExecutor.execute(() -> {
            try {
                ...
                try {
                    AutoAgentExecuteResultEntity err = AutoAgentExecuteResultEntity.createErrorResult(
                            "执行异常：" + e.getMessage(),
                            requestParameter.getSessionId() != null ? requestParameter.getSessionId() : "");
                    emitter.send("data: " + JSON.toJSONString(err) + "\n\n");
