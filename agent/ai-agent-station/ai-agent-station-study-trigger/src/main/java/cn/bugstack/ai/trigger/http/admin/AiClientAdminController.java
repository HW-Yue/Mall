...
import cn.bugstack.ai.api.config.AgentStrategyConfig;
...
public class AiClientAdminController implements IAiClientAdminService {
    ...
    @Override
    @GetMapping("/strategy-slots")
    public Response<AiClientStrategySlotsResponseDTO> getStrategySlots(@RequestParam("strategy") String strategy) {
        try {
            log.info("按策略查询槽位请求：strategy={}", strategy);
            AgentStrategyConfig.SlotDef slotDef = AgentStrategyConfig.getSlotsByStrategy(strategy);
            AiClientStrategySlotsResponseDTO dto = AiClientStrategySlotsResponseDTO.builder()
                    .strategy(slotDef.getStrategy())
                    .requiredSlots(slotDef.getRequiredSlots())
                    .optionalSlots(slotDef.getOptionalSlots())
                    .build();
            return Response.<AiClientStrategySlotsResponseDTO>builder()
                    .code(ResponseCode.SUCCESS.getCode())
                    .info(ResponseCode.SUCCESS.getInfo())
                    .data(dto)
                    .build();
        } catch (Exception e) {
            log.error("按策略查询槽位失败", e);
            return Response.<AiClientStrategySlotsResponseDTO>builder()
                    .code(ResponseCode.UN_ERROR.getCode())
                    .info(ResponseCode.UN_ERROR.getInfo())
                    .data(null)
    ...
    public Response<List<AiClientSimpleItemDTO>> listByType(@RequestParam("clientType") String clientType) {
        try {
            log.info("按类型查询客户端列表请求：clientType={}", clientType);
            if (!StringUtils.hasText(clientType)) {
                return Response.<List<AiClientSimpleItemDTO>>builder()
                        .code(ResponseCode.ILLEGAL_PARAMETER.getCode())
                        .info("clientType 不能为空")
                        .data(Collections.emptyList())
                        .build();
            }
            List<AiClient> list = aiClientDao.queryByClientType(clientType);
            List<AiClientSimpleItemDTO> dtos = list.stream()
                    .map(c -> AiClientSimpleItemDTO.builder()
    ...
    @Override
    @GetMapping("/query-by-client-type")
    public Response<List<AiClientResponseDTO>> queryByClientType(@RequestParam("clientType") String clientType) {
        try {
            log.info("按 client_type 查询客户端完整列表请求：clientType={}", clientType);
            if (!StringUtils.hasText(clientType)) {
                return Response.<List<AiClientResponseDTO>>builder()
                        .code(ResponseCode.ILLEGAL_PARAMETER.getCode())
                        .info("clientType 不能为空")
            ...
            List<AiClient> list = aiClientDao.queryByClientType(clientType);
            List<AiClientResponseDTO> dtos = list.stream()
                    .map(this::convertToAiClientResponseDTO)
                    .collect(Collectors.toList());
            return Response.<List<AiClientResponseDTO>>builder()
                    .code(ResponseCode.SUCCESS.getCode())
            ...
            log.error("按 client_type 查询客户端列表失败", e);
            return Response.<List<AiClientResponseDTO>>builder()
                    .code(ResponseCode.UN_ERROR.getCode())
                    .info(ResponseCode.UN_ERROR.getInfo())
                    .data(null)
