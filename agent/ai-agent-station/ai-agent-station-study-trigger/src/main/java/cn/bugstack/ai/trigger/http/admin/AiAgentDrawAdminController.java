...
public class AiAgentDrawAdminController implements IAiAgentDrawAdminService {
    ...
    /** 按策略返回 client_type 槽位编码数组，供前端二级联动第一步使用 */
    @GetMapping("/client-types-by-strategy")
    public Response<List<String>> clientTypesByStrategy(@RequestParam("strategy") String strategy) {
        try {
            AgentStrategyConfig.SlotDef slotDef = AgentStrategyConfig.getSlotsByStrategy(strategy);
            List<String> slots = new ArrayList<>(slotDef.getRequiredSlots());
            slots.addAll(slotDef.getOptionalSlots());
            ...
            log.error("按策略查询 client_type 槽位失败", e);
            return Response.<List<String>>builder()
