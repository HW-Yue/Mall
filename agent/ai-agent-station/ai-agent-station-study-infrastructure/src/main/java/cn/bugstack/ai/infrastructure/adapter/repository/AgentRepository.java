...
public class AgentRepository implements IAgentRepository {
    ...
    public List<AiClientApiVO> queryAiClientApiVOListByApiIds(List<String> apiIdList) {
        ...
        List<AiClientApi> list = aiClientApiDao.queryByApiIds(apiIdList);
        return list.stream()
                .map(api -> AiClientApiVO.builder()
                        .apiId(api.getApiId())
        ...
        for (String clientId : clientIdList) {
            ...
            for (AiClientConfig config : configs) {
                if (AI_CLIENT_MODEL.getCode().equals(config.getTargetType()) && config.getStatus() == 1) {
                    ...
                    AiClientModel model = aiClientModelDao.queryByModelId(modelId);
                    if (model != null && model.getStatus() == 1) {
                        // MCP 改为策略绑定：由 client 直连 tool_mcp，此处不再查 model -> tool_mcp，toolMcpIds 由装配节点从 context 的客户端 MCP 列表获取
                        AiClientModelVO modelVO = AiClientModelVO.builder()
                                .modelId(model.getModelId())
                                .apiId(model.getApiId())
                                .modelName(model.getModelName())
                                .modelType(model.getModelType())
                                .toolMcpIds(Collections.emptyList())
                                .build();
    ...
    public List<AiClientAdvisorVO> AiClientAdvisorVOByClientIds(List<String> clientIdList) {
        ...
        for (String clientId : clientIdList) {
            ...
            for (AiClientConfig config : configs) {
                ...
                // ChatMemory 类型若未解析到配置，使用默认值，避免 createAdvisor 时 NPE
                if ("ChatMemory".equals(aiClientAdvisor.getAdvisorType()) && chatMemory == null) {
                    chatMemory = AiClientAdvisorVO.ChatMemory.builder().maxMessages(20).build();
                }
    ...
    public void createTagOrder(AiRagOrderVO aiRagOrderVO) {
        ...
        // 业务唯一标识：为空时生成 32 位无横线 UUID
        if (!hasText(aiRagOrder.getRagId())) {
