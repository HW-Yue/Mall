...
class RocketMqConsumerThreadPoolRefresherTest {
    ...
    void shouldUpdateRunningConsumerThreadPoolOnStartupAndRefresh() {
        String consumerGroup = "CG_GROUP_BUY_ORDER_PAID";
        ...
        beanFactory.addBean("orderPaidGroupBuyListenerContainer", container);
        ...
        refresher.onApplicationEvent(new EnvironmentChangeEvent(this, Set.of(
                "app.rocketmq.consumer-thread-pool.consumers.CG_GROUP_BUY_ORDER_PAID.consume-thread-number",
                "app.rocketmq.consumer-thread-pool.consumers.CG_GROUP_BUY_ORDER_PAID.consume-thread-max"
        )));
    ...
    private static void printState(String phase, DefaultMQPushConsumer consumer, ThreadPoolExecutor executor) {
        System.out.printf(
                "[RocketMqConsumerThreadPoolRefresherTest-group-buy] phase=%s consumerMin=%d consumerMax=%d executorCore=%d executorMax=%d%n",
                phase,
