-- ============================================================
-- mall_db：商城服务 — 商品目录 + 活动类型字典
-- 仅存基础商品信息，不含营销活动、折扣、订单
-- ============================================================

CREATE DATABASE IF NOT EXISTS `mall_db` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE `mall_db`;
...
INSERT INTO `category` (`name`, `code`, `sort_order`)
VALUES ('AI模型', 'ai_model', 10), ('存储资源', 'storage', 20);


-- ------------------------------------------------------------
-- sku：商品基础信息（原始价格为官方定价，不含营销折扣）
-- ------------------------------------------------------------
...
INSERT INTO `sku` (`goods_id`, `goods_name`, `original_price`, `category_id`, `total_stock`)
VALUES
  ('1001', 'GPT-4 100万Token',     199.00, 1, 88),
  ('1002', 'GPT-4 500万Token',     899.00, 1, 100),
  ('1003', 'Claude 100万Token',    159.00, 1, 88),
  ('1004', '文心一言 200万Token',   99.00, 1, 88),
  ('1005', '通义千问 100万Token',   89.00, 1, 88),
  ('2001', '云存储 100MB',           9.90, 2, 500),
  ('2002', '云存储 200MB',          18.00, 2, 88),
  ('2003', '云存储 500MB',          39.00, 2, 88),
  ('2004', '云存储 1GB',            69.00, 2, 88),
  ('2005', '云存储 1TB',           299.00, 2, 88);


-- ------------------------------------------------------------
-- sku_resource_detail：商品资源规格（token数 / 存储容量）
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- activity_type：活动类型字典（mall 统一维护，各营销服务读取）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `activity_type`;
CREATE TABLE `activity_type` (
  `id`          int unsigned NOT NULL AUTO_INCREMENT COMMENT '自增ID',
  `type_name`   varchar(32)  NOT NULL COMMENT '活动名称，如：秒杀、拼团',
  `type_code`   varchar(32)  NOT NULL COMMENT '活动编码，如：seckill、group_buy',
...
INSERT INTO `activity_type` (`type_name`, `type_code`)
VALUES ('秒杀', 'seckill'), ('拼团', 'group_buy');
