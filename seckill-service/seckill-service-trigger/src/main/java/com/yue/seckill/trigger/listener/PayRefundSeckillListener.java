 ...
 *
 * <p>由 order-service 在已支付订单退款时发出（事务消息）。本服务收到后：
 * 1. SETNX 幂等锁，避免重投复加；
 * 2. 反查 outTradeNo → activityId/productId 上下文（OrderPaidSeckillListener 在真实扣减成功时写入）；
    ...
    public void onMessage(String message) {
        ...
        try {
            ...
            if (StringUtils.isBlank(outTradeNo)) {
                log.error("[秒杀退款] 消息缺少 outTradeNo: {}", message);
                return;
            }

            // 幂等锁
            String dedupKey = REFUNDED_KEY_PREFIX + outTradeNo;
            Boolean acquired = stringRedisTemplate.opsForValue().setIfAbsent(
                    dedupKey, String.valueOf(System.currentTimeMillis()), DEDUP_TTL_HOURS, TimeUnit.HOURS);
            if (acquired == null || !acquired) {
                log.info("[秒杀退款] 幂等命中，跳过 outTradeNo:{}", outTradeNo);
                return;
            }

            // 反查库存上下文
            String ctx = stringRedisTemplate.opsForValue().get(ORDER_BY_TRADE_KEY_PREFIX + outTradeNo);
            if (StringUtils.isBlank(ctx)) {
                log.warn("[秒杀退款] 库存上下文已不存在，活动可能已结束，跳过 outTradeNo:{}", outTradeNo);
                return;
            }
            String[] parts = ctx.split(":");
            if (parts.length < 2 || StringUtils.isAnyBlank(parts[0], parts[1])) {
                log.error("[秒杀退款] 库存上下文格式异常 outTradeNo:{} ctx:{}", outTradeNo, ctx);
                return;
            }
            Long activityId;
            try {
                ...
                log.error("[秒杀退款] activityId 解析失败 outTradeNo:{} ctx:{}", outTradeNo, ctx, e);
                return;
            }
            ...
            if (restoreResult == 0) {
                log.warn("[秒杀退款] Redis 库存 key 已过期，跳过 outTradeNo:{} activityId:{} productId:{}",
                        outTradeNo, activityId, productId);
                return;
            }
            log.info("[秒杀退款] Redis 库存恢复 result:{} outTradeNo:{} activityId:{} productId:{}",
                    restoreResult, outTradeNo, activityId, productId);
