...
class RocketMqConsumerThreadPoolRefresherTest {
    ...
    void shouldUpdateRunningConsumerThreadPoolOnStartupAndRefresh() {
        String consumerGroup = "CG_ORDER_PAID_SECKILL";
        ...
        beanFactory.addBean("orderPaidSeckillListenerContainer", container);
        ...
        refresher.onApplicationEvent(new EnvironmentChangeEvent(this, Set.of(
                "app.rocketmq.consumer-thread-pool.consumers.CG_ORDER_PAID_SECKILL.consume-thread-number",
                "app.rocketmq.consumer-thread-pool.consumers.CG_ORDER_PAID_SECKILL.consume-thread-max"
        )));
    ...
    private static void printState(String phase, DefaultMQPushConsumer consumer, ThreadPoolExecutor executor) {
        System.out.printf(
                "[RocketMqConsumerThreadPoolRefresherTest-seckill] phase=%s consumerMin=%d consumerMax=%d executorCore=%d executorMax=%d%n",
                phase,
